...
public class Administrador {
	...
	/**
	 * Devuelve false o true segun la existencia o no de un empleado ya cargado
	 * con el nro de legajo pasado como parametro
	 * 
	 * @param nroLegajo
	 * @return
	 */
	public boolean existeEmpleado(int nroLegajo) {
	...
	/**
	 * Elimina un usuario de la base de datos, retorna true si tu exito y false
	 * en caso contrario
	 * 
	 * @param usuario
	 * @return
	...
	public List<EmpleadoDTO> getEmpleados() {
		...
		if (!listaEmpleados.isEmpty()) {
			for (int i = 0; i < listaEmpleados.size(); i++) {
				...
				if (!listaEmpleados.get(i).getEmpleadosForEmpleado().isEmpty()) {
					Iterator<Empleado> listita = listaEmpleados.get(i).getEmpleadosForEmpleado().iterator();

					while (listita.hasNext()) {

						Empleado emp = listita.next();
						EmpleadoDTO empDTO = new EmpleadoDTO();
						empDTO.setNroLegajo(emp.getNroLegajo());
						empDTO.setApellido(emp.getApellido());
						empDTO.setNombre(emp.getNombre());
						empDTO.setPuesto(emp.getPuesto());
						nuevo.getListaEmpACargo().add(empDTO);
					}
				}
